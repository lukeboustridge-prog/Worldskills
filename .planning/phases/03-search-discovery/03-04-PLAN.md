...
<task type="auto">
  ...
  <files>src/lib/queries/related-descriptors.ts</files>
  <action>
...
Wait, the dynamic SQL with excludeId won't work that way. Let me fix:
...
async function test() {
  ...
  if (!sample) { console.log('No descriptors found'); return; }

  console.log('Source:', sample.criterionName, '(', sample.skillName, ')');

  // Test by ID
  const related = await getRelatedDescriptors(sample.id);
  console.log('Related by ID:', related.length);
  related.forEach(r => console.log('  -', r.similarityScore.toFixed(2), r.skillName, r.criterionName.substring(0, 50)));

  // Test by text
  const byText = await getRelatedByCriterionName(sample.criterionName);
  console.log('Related by text:', byText.length);
}
...
<task type="auto">
  ...
  <files>None (verification only)</files>
  <action>
Validate the cross-skill discovery feature by testing real patterns:
...
async function test() {
  ...
  if (!safety) { console.log('No safety descriptor found'); return; }
...
This validates the primary use case: "See how other skills describe safety" from SEARCH-08 requirement.
  ...
  <verify>
At least one related descriptor is found from a DIFFERENT skill than the source, demonstrating cross-skill discovery capability.
  </verify>
