...
async function test() {
  ...
  // Verify counts are lower when filtered
...
<task type="auto">
  <name>Task 3: Verify filters combine with search correctly</name>
  <files>None (verification only)</files>
  <action>
Test that applying filters narrows search results (AND combination) rather than replacing them:
...
async function test() {
  ...
  console.log('Filter narrows results:', filtered.total <= searchOnly.total);
...
- Results still ranked: true
  ...
  <verify>
All three assertions pass: filters narrow results, all results match filter, results remain ranked by relevance.
  </verify>
  <done>Filters confirmed to narrow search results (AND combination), search relevance ranking preserved when filters applied</done>
</task>
...
- Performance remains <100ms for paginated queries
</success_criteria>
