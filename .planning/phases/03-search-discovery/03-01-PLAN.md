...
<task type="auto">
  ...
  <files>prisma/migrations/YYYYMMDDHHMMSS_add_fts_indexes/migration.sql, scripts/verify-fts-index.ts</files>
  <action>
...
4. Create a standalone verification script at `scripts/verify-fts-index.ts`:
...
Run `npx ts-node scripts/verify-fts-index.ts` - should output "FTS index found".
  </verify>
...
2. GIN index exists: `npx ts-node scripts/verify-fts-index.ts` passes
