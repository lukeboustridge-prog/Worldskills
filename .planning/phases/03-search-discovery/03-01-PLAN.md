...
"
```
Should return results with non-null rank values.
  </verify>
...
1. Run timing tests for common query patterns:
```bash
npx ts-node -e "
import { searchDescriptors } from './src/lib/search-descriptors';
...
2. If any query exceeds 100ms, check that the GIN index is being used:
```sql
EXPLAIN ANALYZE
...
3. Document performance results in the summary.
  </action>
  <verify>
...
3. Search function works: `searchDescriptors({ query: 'safety' })` returns ranked results
