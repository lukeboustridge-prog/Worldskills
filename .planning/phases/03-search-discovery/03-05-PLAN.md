...
<task type="auto">
  <name>Task 2: Create search page with URL-driven state</name>
...
<task type="auto">
  <name>Task 3: Create debounced search input with URL sync</name>
  <files>src/app/descriptors/components/SearchInput.tsx</files>
  <action>
...
- Uses existing shadcn/ui Input component
  </action>
  <verify>
...
Refresh page with ?q=safety - input should show "safety".
  ...
  <done>Debounced search input synced to URL, 300ms delay, clear button, state persists on refresh</done>
</task>

<task type="auto">
  <name>Task 4: Create filter panel, pagination, and descriptor list with copy functionality</name>
  <files>src/app/descriptors/components/FilterPanel.tsx, src/app/descriptors/components/Pagination.tsx, src/app/descriptors/components/DescriptorList.tsx</files>
  <action>
...
Key implementation for copy functionality:
...
- Each performance level (excellent, good, pass) has its own copy button
- Criterion name also copyable
  </action>
  <verify>
...
All filters combine in URL: ?q=safety&skill=Welding&page=2
Hover over a descriptor card - copy buttons appear.
Click copy button - text copied to clipboard, checkmark appears briefly.
  </verify>
  <done>Filter panel, pagination, and descriptor list components created, all synced to URL state, copy-to-clipboard functionality working</done>
</task>
