...
function buildStorageKey(params: { senderId: string; filename: string }) {
  const safeName = normaliseFileName(params.filename);
  const timestamp = Date.now();
  const slug = randomUUID().slice(0, 8);
  return `emails/${params.senderId}/${timestamp}-${slug}-${safeName}`;
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Only SA and Secretariat can send emails
    if (user.role !== Role.SA && user.role !== Role.Secretariat && !user.isAdmin) {
      return NextResponse.json(
        { error: "forbidden", message: "You do not have permission to upload email attachments." },
    ...
    console.error("[emails/attachments/upload] failed", err);
