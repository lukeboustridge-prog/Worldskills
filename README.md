...
- Node.js 20.11 or newer (aligns with Vercel's Node 20 runtime)
...
4. Trigger a deployment. The build pipeline runs `pnpm build`, which in turn executes `prisma generate`, `prisma migrate deploy`, and `next build` so your Neon database is migrated during the build step.
   - The repository pins pnpm 9.12 and configures Vercel to activate it via Corepack on the Node.js 20 runtime declared in `vercel.json`.
...
- The included `vercel.json` enforces the correct Next.js build output (`.next`), activates pnpm 9.12 via Corepack, and runs functions on the Node 20 runtime so Vercel serves the application runtime instead of a static 404.
