...
- pnpm 8.15+ (installed via [`corepack`](https://nodejs.org/api/corepack.html) or `npm i -g pnpm`)
...
4. Trigger a deployment. The build pipeline runs `pnpm build`, which in turn executes `prisma generate`, `prisma migrate deploy`, and `next build` so your Neon database is migrated during the build step.
   - The repository pins pnpm 8.15 so Vercel can install dependencies reliably on the default Node.js 18 runtime.
