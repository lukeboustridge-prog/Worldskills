...
- Node.js 20.11 or newer (matches the runtime required by pnpm 10 on Vercel)
...
4. Trigger a deployment. The build pipeline runs `pnpm build`, which in turn executes `prisma generate`, `prisma migrate deploy`, and `next build` so your Neon database is migrated during the build step.
   - Vercel automatically selects a Node.js 20.x runtime to satisfy the package `engines` requirement, avoiding the incompatibility between pnpm 10 and Node 18 that caused prior install failures.
