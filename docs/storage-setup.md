...
6. Verify the configuration by visiting `/api/storage/health`; the response should be `{ "ok": true }` and include `runtime: "nodejs"` and `diagnostic: "blob_verified"` in Preview/Development builds.
